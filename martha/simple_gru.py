...
class SimpleGRU(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.invariant_projection = nn.Linear(
            ...
            self.config["encoder_input_size"], 
        )
        self.invariant_encoder = nn.GRU(
            input_size=self.config["encoder_input_size"],
    ...
    def encode_inv(self, inputs, state, seq_lens):
        ...
        self._invariant_features = torch.reshape(
            tmp2_inv[:,-1:,:], [-1, self.config["encoder_hidden_size"]]
        )
